...
/// Scraper specifically designed for ntv.com.tr
...
class NtvScraper {
  ...
  /// Base URL for NTV
  static const String baseUrl = 'https://www.ntv.com.tr';

  /// Source ID for NTV
  static const String sourceId = 'ntv';

  /// Source name for display
  static const String sourceName = 'NTV';

  /// Number of concurrent article fetches (legacy mode only)
  static const int _concurrentFetches = 5;

  NtvScraper({Dio? dio, ParallelScraperService? parallelService})
      : _dio = dio ??
            Dio(
              BaseOptions(
  ...
  /// Scrape articles from NTV main page
  ...
  Future<List<NewsArticle>> _scrapeWithTrueParallel(
    ...
    return _parallelService.scrapeArticlesParallel(
      ...
      source: source,
      config: config,
      parseFunction: parseNtvArticle, // TOP-LEVEL function from scraper_parsing_functions.dart
    );
  }
  ...
  List<Map<String, String>> _extractArticleLinks(Document document, int maxCount) {
    ...
    // 1. Look for article links (NTV uses various patterns)
    articleElements = document.querySelectorAll(
      'a[href*="/turkiye/"], a[href*="/dunya/"], a[href*="/ekonomi/"], a[href*="/spor/"], a[href*="/teknoloji/"], a[href*="/saglik/"], a[href*="/egitim/"]',
    );

    // 2. Fallback: Look for any internal links that look like articles
    if (articleElements.isEmpty) {
      final allLinks = document.querySelectorAll('a[href^="/"]');
      articleElements = allLinks.where((el) {
        final href = el.attributes['href'] ?? '';
        return RegExp(r'^/[a-z-]+/[a-z0-9-]+-\d{7}$').hasMatch(href);
      }).toList();
    }
    ...
    for (final element in articleElements) {
      ...
      if (url.contains('/video/') ||
          ...
          url.contains('/canli-yayin') ||
          url == baseUrl ||
          url == '$baseUrl/') {
        continue;
      }

      final articleIdPattern = RegExp(r'-(\d{7})$');
      final idMatch = articleIdPattern.firstMatch(url);
      if (idMatch == null) continue;

      var title = element.attributes['title'] ?? element.text.trim();
      title = title.replaceAll(RegExp(r'\s*\|\s*NTV\s*Haber\s*$'), '').trim();
      ...
      final categoryMatch = RegExp(r'^/([^/]+)/').firstMatch(url.replaceFirst(baseUrl, ''));
  ...
  Future<NewsArticle?> _scrapeArticleDetail({
    ...
    try {
      ...
      final parsed = parseNtvArticle(response.data, url);
